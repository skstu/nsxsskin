...
namespace shared {
	...
	std::time_t Win::Time::FileTimeToStamp(const FILETIME& ft) {
		std::time_t result = 0;
		...
		result = static_cast<std::time_t>((ull.QuadPart - 116444736000000000ULL) / 10000000ULL);

		return result;
	}
	std::time_t Win::Time::SystemTimeToStamp(const SYSTEMTIME& st) {
		std::time_t result = 0;
		do {
			FILETIME ft;
			if (FALSE == SystemTimeToFileTime(&st, &ft))
				break;
			result = FileTimeToStamp(ft);
		} while (0);
		return result;
	}
	...
	std::string Win::Time::TimeSystemString(bool IsMilliseconds /*= false*/) {
		...
		char tmp[512] = { 0 };
		if (IsMilliseconds) {
			sprintf_s(tmp, sizeof(tmp), "%04d/%02d/%02d %02d:%02d:%02d.%03lld",
