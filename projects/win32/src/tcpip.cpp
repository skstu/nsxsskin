...
/* Note: could also use malloc() and free() */
   ...
   while (pCurrAddresses) {
    ...
    if (pCurrAddresses->PhysicalAddressLength != 0)
     for (int i = 0; i < (int)pCurrAddresses->PhysicalAddressLength; i++) {
      char tmp[128] = { 0 };
      sprintf_s(tmp, sizeof(tmp), "%02X", (int)pCurrAddresses->PhysicalAddress[i]);
 ...
 do {
			...
			// Prepare for getting the IP address
			...
			// Iterate through the results and get the IP address
			for (ptr = result; ptr != NULL; ptr = ptr->ai_next) {
				if (ptr->ai_family == AF_INET) {
					sockaddr_ipv4 = (struct sockaddr_in*)ptr->ai_addr;
					inet_ntop(AF_INET, &sockaddr_ipv4->sin_addr, ip_string_buffer, sizeof(ip_string_buffer));
					std::cout << "IP Address: " << ip_string_buffer << std::endl;
				}
			}
