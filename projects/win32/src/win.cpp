...
namespace shared {
	...
	const char STANDARD_COMPONECT_INTERFACE_SIGNATURE_INIT_V2[] = "interface_init";
	const char STANDARD_COMPONECT_INTERFACE_SIGNATURE_UNINIT_V2[] = "interface_uninit";
...
#pragma comment(lib, "advapi32.lib")
	...
	HWND Win::GetHWNDFromRECT(const RECT& rect) {
		// 计算 RECT 的中心点
		POINT pt;
	...
	void Win::EnumChildWindowsByValid(const HWND& hProcessWnd,
		std::vector<HWND>& hwnds) {
		hwnds.clear();
		do {
			if (!hProcessWnd)
				break;
			if (!IsWindow(hProcessWnd))
				break;
			::EnumChildWindows(
				hProcessWnd,
				[](HWND hwnd, LPARAM route) -> BOOL {
					auto hwnds =
						reinterpret_cast<std::vector<HWND> *>(route);
	...
	void Win::EnumChildWindows(const HWND& input, const std::function<void(const HWND&, bool&)>& cb) {
		if (!input)
			return;
		::EnumChildWindows(
			input,
			[](HWND hwnd, LPARAM route) -> BOOL {
				auto pOutInfos =
					reinterpret_cast<std::function<void(const HWND&, bool&)>*>(route);
				if (pOutInfos) {
					bool itbreak = false;
					(*pOutInfos)(hwnd, itbreak);
					if (itbreak)
						return FALSE;
				}
				return TRUE;
	...
	void Win::EnumChildWindows(const HWND& input, std::set<HWND>& output) {
		output.clear();
		if (!input)
			return;
		::EnumChildWindows(
			...
			[](HWND hwnd, LPARAM route) -> BOOL {
				auto pOutInfos =
					reinterpret_cast<std::set<HWND> *>(route);
				if (pOutInfos)
					pOutInfos->emplace(hwnd);
				return TRUE;
			},
			(LPARAM)&output);
	}
	...
	std::string Win::GenerateHexBuffer(const std::string& buffer,
		...
		do {
			...
			for (auto it = buffer.begin(); it != buffer.end(); ++it) {
				...
				char tmp[64] = { 0 };
				sprintf_s(tmp, sizeof(tmp), "%02X", static_cast<std::uint8_t>(*it));
				stream << tmp;
#if 0
			...
			char tmp[256] = { 0 };
			sprintf_s(tmp, sizeof(tmp), "static const unsigned char %s[]=", variable.c_str());
			result.insert(0, tmp);
	...
	void Win::GetNumberOfProcessors(DWORD& numCores) {
		SYSTEM_INFO sysInfo;
		GetSystemInfo(&sysInfo);
		numCores = sysInfo.dwNumberOfProcessors;
	}
	void Win::GetLogicalProcessorInfo(DWORD& physicalCoreCount, DWORD& logicalProcessorCount) {
#if defined(_WIN64)
		DWORD length = 0;
		...
		for (DWORD i = 0; i < count; i++) {
			if (buffer[i].Relationship == RelationProcessorCore) {
				...
				logicalProcessorCount += (DWORD)__popcnt64(buffer[i].ProcessorMask);
			}
		}
	...
	bool Win::CreateDirectoryA(const std::string& create_directory_path) {
		...
		do {
			...
			do {
				if (::access(temp_directory_path.c_str(), 0) == 0) {
	...
	std::wstring Win::UrlFixedW(const std::wstring& inputUrlOrPath) {
		std::wstring result{ inputUrlOrPath };
		if (result.empty())
			return result;
	...
	std::wstring Win::GetProgramFiles() {
		std::wstring result;
		PWSTR path = NULL;
		do {
	...
	std::wstring Win::GetProgramFilesX86() {
		std::wstring result;
		PWSTR path = NULL;
		do {
			if (FAILED(SHGetKnownFolderPath(FOLDERID_ProgramFilesX86, 0, NULL, &path)))
				break;
			result = path;
		} while (0);
		if (path) {
	...
	void* Win::DllGetModuleHandle() {
#if defined(_WIN32) || defined(_WIN64)
		MEMORY_BASIC_INFORMATION mbi;
		VirtualQuery(Win::DllGetModuleHandle, &mbi, sizeof(mbi));
		return (void*)(mbi.AllocationBase);
#else
		Dl_info dl_info;
		dladdr((void*)GetCurrentModule, &dl_info);
	...
	bool Win::ProgramInstanceMark(const std::string& ProgramName) {
		...
		do {
			if (ProgramName.empty())
				break;
			hMutex = ::CreateMutexA(NULL, FALSE, ProgramName.c_str());
...
#include <intrin.h>
	...
	void Win::CommandLineArgsW(tfCommandLineNodeW& out) {
		...
		do {
			...
			File::ParseW(commandline, L' ', argvs);
			if (argvs.empty())
				break;
			for (auto& argv : argvs) {
				auto separator = ::StrStrIW(argv.c_str(), L"=");
				auto node_begin = ::StrStrIW(argv.c_str(), L"--");
				if (separator && node_begin) {
					std::vector<std::wstring> parses;
					shared::Win::File::ParseW(node_begin, L'=', parses);
					if (parses.size() > 1)
						out.emplace(IConv::ToLowerW(parses[0]), parses[1]);
				}
				else if (node_begin) {
					std::wstring wstrK(node_begin);
					wstrK = wstrK.substr(0, wstrK.find(L' '));
					out.emplace(IConv::ToLowerW(wstrK), L"");
				}
	...
	void Win::CommandLineArgsA(std::map<std::string, std::string>& out_param_map) {
		out_param_map.clear();
		for (int i = 0; i < __argc; ++i) {
			...
			do {
				...
				if (k && v) {
					...
					std::string strK(k);
					strK = strK.substr(0, strK.find(' '));
					out_param_map.emplace(shared::IConv::ToLowerA(strK), "");
				}
	...
	bool Win::ParseCommandLineParametersMulti(const std::string& commandline,
		...
		do {
			...
			for (auto& argv : argvs) {
				...
				if (parses.size() != 2)
					continue;
				const auto key = IConv::ToLowerA(parses[0]);
				auto exists = out.find(key);
				if (exists != out.end()) {
					exists->second.emplace_back(parses[1]);
				}
				else {
					out.emplace(IConv::ToLowerA(parses[0]), std::vector<std::string>{parses[1]});
				}
			}
			result = !out.empty();
		} while (0);
	...
	bool Win::ParseCommandLineParametersMultiW(const std::wstring& commandline,
		...
		do {
			...
			for (auto& argv : argvs) {
				...
				if (parses.size() != 2)
					continue;
				const auto key = IConv::ToLowerW(parses[0]);
				auto exists = out.find(key);
				if (exists != out.end()) {
					exists->second.emplace_back(parses[1]);
				}
				else {
					out.emplace(IConv::ToLowerW(parses[0]), std::vector<std::wstring>{parses[1]});
				}
			}
			result = !out.empty();
		} while (0);
	...
	bool Win::ParseCommandLineParametersW(const std::wstring& commandline,
		...
		do {
...
#else
			char tmp[256] = { 0 };
			sprintf_s(tmp, sizeof(tmp), "S-%d-%d", SID_REVISION,
				pSID_IDENTIFIER_AUTHORITY->Value[5]);
			outSid = tmp;
#endif
			...
			for (int i = 0; i < SidSubAuthorityCount; ++i) {
				...
				if (!pGetSidSubAuthority)
					break;
				char tmp[64] = { 0 };
				sprintf_s(tmp, sizeof(tmp), "%lu", *pGetSidSubAuthority);
				//outSid.append("-").append(std::format("{}", *pGetSidSubAuthority));
				outSid.append("-").append(tmp);
			}
			result = !outSid.empty();
		} while (0);
		return result;
	...
	bool Win::GetCurrentUserSid(std::string& outSid) {
		...
		do {
			...
			/*outSid = std::format("S-{}-{}", SID_REVISION,
				pSID_IDENTIFIER_AUTHORITY->Value[5]);*/
			char tmp[256] = { 0 };
			sprintf_s(tmp, 256, "S-%d-%d", SID_REVISION,
				pSID_IDENTIFIER_AUTHORITY->Value[5]);
			outSid = tmp;
			auto pGetSidSubAuthorityCount = ::GetSidSubAuthorityCount(UserSid);
			if (!pGetSidSubAuthorityCount)
			...
			for (int i = 0; i < SidSubAuthorityCount; ++i) {
				...
				if (!pGetSidSubAuthority)
					break;
				char tmp[64] = { 0 };
				sprintf_s(tmp, 64, "%lu", *pGetSidSubAuthority);
				//outSid.append("-").append(std::format("{}", *pGetSidSubAuthority));
				outSid.append("-").append(tmp);
			}
			result = !outSid.empty();
...
#pragma pack(pop)
	...
	bool Win::CreateShortcutW(
		const std::wstring& szPath,
		const std::wstring& szLink,
		const std::wstring& szIcoPath,
		const std::wstring& szArgument) {
		HRESULT comret = S_FALSE;
		IShellLinkW* pIShellLinkW = nullptr;
		IPersistFile* pIPersistFile = nullptr;
		bool result = false;
		do {
			...
			if (comret != S_OK)
				break;
			comret =
				pIShellLinkW->SetPath(szPath.c_str());
			if (comret != S_OK)
				break;
			comret = pIShellLinkW->SetWorkingDirectory(PathFixedW(szPath).c_str());
			if (comret != S_OK)
				break;
			if (!szIcoPath.empty())
				comret = pIShellLinkW->SetIconLocation(szIcoPath.c_str(), 0);
			if (comret != S_OK)
				break;
			if (!szArgument.empty())
				comret = pIShellLinkW->SetArguments(szArgument.c_str());
			if (comret != S_OK)
				break;
			comret =
				pIShellLinkW->QueryInterface(IID_IPersistFile, (void**)&pIPersistFile);
			if (comret != S_OK)
				break;
			comret = pIPersistFile->Save(szLink.c_str(),
				TRUE);
			if (comret != S_OK)
				break;
...
{EN_FILE_TYPE::_TAR,
	...
	DWORD Win::GetActiveConsoleSessionId() {
		return WTSGetActiveConsoleSessionId();
	}

	bool Win::GetSessionIdFromProcessId(const DWORD& pid, DWORD& sessionId) {
		bool result = 0;
		do {
