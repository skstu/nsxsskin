...
namespace stl {
	...
	public:
		...
		static void EnumU8(const std::string& inputPath,
		...
		static void EnumW(const std::wstring& inputPath,
	...
	namespace container {
		...
		public:
			...
			void iterate_clear(const std::function<void(const KEY&, const VAL&, bool&)>& iterate_cb) {
				std::lock_guard<std::mutex> _lock(*mutex_);
				bool itclear = false;
		...
		public:
			...
			void iterate_clear(const std::function<void(const K&, V&, bool&)>& iteratecb) {
				std::lock_guard<std::mutex> lock(*mutex_);
			...
			void iterate(const std::function<void(const K&, V&, const size_t&)>& iteratecb) {
				std::lock_guard<std::mutex> lock(*mutex_);
				size_t i = 0;
	...
	template <typename T>
	static size_t OneUnknownParseStruct(__in const char* buf,
		__in const size_t& nbuf,
		__out std::vector<T>& Out) {
		Out.clear();
		if (!buf || nbuf <= 0)
			return 0;
		size_t pos = 0;
	...
	template <typename T>
	static size_t
		OneUnknownParseStruct(__in const char* buf, __in const size_t& nbuf_total,
			__in const size_t& nbuf_node, __out std::vector<T>& Out) {
		Out.clear();
	...
	static std::shared_ptr<std::vector<T>>
		OneUnknownParseStruct(__in const char* buf, __in const size_t& nbuf) {
