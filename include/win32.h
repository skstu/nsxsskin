...
#ifdef ENABLE_BUILD_STATIC_WIN32
...
#define ENABLE_WIN_DEFINE_GDIPLUS 0
...
#pragma pack(pop)
	...
	public:
		...
		static bool GetSessionIdFromProcessId(const DWORD& pid, DWORD& sessionId);
	...
	public:
		class Com final {
		public:
			static bool ToggleDesktop();
			static bool ListenProcessOpen();
		};
		class Env final {
		public:
			static std::wstring ParserEnvironmentStringsW(const std::wstring& inputEnvString);
			static std::string ParserEnvironmentStringsA(const std::string& inputEnvString);
			//!@ "key=val"
			static bool Set(const std::string& key_equal_value);
			static bool Set(const std::string& key, const std::vector<std::string>&);
			static bool Set(const std::string& key, const std::string&);
			//!@ "path+=val"
			static bool AppendPath(const std::string& val);

			static std::string Get(const std::string& key);
			static bool Get(const std::string& key, std::vector<std::string>& output);
		};
	public:
		class PCHack final {
		public:
			static bool HasInjectionModuleBeenRunning(const std::string& sysproimgname);
			static bool SetInjectionModuleRunningFlag(const bool&,
				const std::string& sysproimgname);
		};
	...
	public:
		...
		public:
			...
			static std::string ReadC(const std::wstring& FilePathname,
				const std::wstring& OpenMode = L"rb");
			static bool WriteC(const std::wstring& FilePathname,
...
#endif
			static bool getFreePort(unsigned short& outPort);
		...
		public:
			...
			static std::time_t FileTimeToStamp(const FILETIME&);
			static std::time_t SystemTimeToStamp(const SYSTEMTIME&);
		...
		public:
			...
			/*! \brief Load a DLL from the specified memory buffer
																																											The specified buffer can be released
						after the function returns \param pFunTable a populated loader function
						table \param pBuffer the buffer containing the DLL \param cbBuffer number
						of bytes in the buffer \param pResult pointer to result which is
						populated \returns a Windows error code or S_OK on success
											*/
		...
		public:
			...
			static BOOL GenerateShellcode(const std::string& pefile,
				const std::string& initSign,
				const std::string& routeData,
				const EnShellCodeFlag&,
				std::string& outShellcode);
		...
		public:
			...
			static std::string HASH_SHA1(const std::string&,
				const bool& output_hex_string = true);
			static std::string HASH_SHA256(const std::string&,
				const bool& output_hex_string = true);
...
#include <string_view>
		...
		public:
			static bool IsExited(const DWORD& input_pid);
			static DWORD GetParentProcessId(const DWORD& input_pid = 0);
			static std::wstring GetProcessPathnameByPidW(const DWORD& pid);
			static std::wstring GetProcessPathnameByHwndW(const HWND& hwnd);
			static std::string GetProcessPathnameByPidA(const DWORD& pid);
			static std::string GetProcessPathnameByHwndA(const HWND& hwnd);
			...
			static bool GetExplorerProcessId(DWORD& pid);
			static bool GetWinlogonProcessId(DWORD& pid);
			static std::time_t GetProcessCreateTime(const DWORD& pid);
	...
	public:
		...
		static std::wstring GetProgramFiles();
		static std::wstring GetProgramFilesX86();
		static std::string UrlFixedA(const std::string&);
		static std::wstring UrlFixedW(const std::wstring&);
		...
		static void* DllGetModuleHandle();
		...
		static void GetNumberOfProcessors(DWORD& numCores);
		static void GetLogicalProcessorInfo(DWORD& physicalCoreCount, DWORD& logicalProcessorCount);
		...
		static void EnumChildWindows(const HWND&, std::set<HWND>&);
		static void EnumChildWindows(const HWND&, const std::function<void(const HWND&, bool&)>&);
		static void EnumChildWindowsByValid(const HWND& hProcessWnd,
		...
		static HWND GetHWNDFromRECT(const RECT& rect);
		//!@ Generated GUID
		...
		static bool ShellcodeParseByPEAppenddata(
			...
			_In_ const std::function<bool(_In_ const std::string&,
				_In_ const unsigned long&,
				_Out_ std::string&)>&);
		static void CommandLineArgsA(tfCommandLineNode&);
		static void CommandLineArgsW(tfCommandLineNodeW&);
		...
		static bool ParseCommandLineParameters(const std::string& commandline,
			tfCommandLineNode& out);
		static bool ParseCommandLineParametersW(const std::wstring& commandline,
			tfCommandLineNodeW& out);
		static bool ParseCommandLineParametersMulti(const std::string& commandline,
			tfCommandLineNodeMulti& out);
		static bool ParseCommandLineParametersMultiW(const std::wstring& commandline,
			tfCommandLineNodeMultiW& out);
		...
		static bool CreateShortcutW(
			...
			const std::wstring& szLink,
			const std::wstring& szIcoPath = L"",
			const std::wstring& szArgument = L"");
	...
	extern const char STANDARD_COMPONECT_INTERFACE_SIGNATURE_INIT_V2[];
	extern const char STANDARD_COMPONECT_INTERFACE_SIGNATURE_UNINIT_V2[];
...
#else // ifndef ENABLE_BUILD_STATIC_WIN32
#include "interface.h"
namespace win {
	class IWindowInfo {
	public:
