...
public:
	...
	static void MadeHttpGetNode(const rapidjson::Value& value, const std::string& path, tfHttpGetNode& nodes) {
		if (value.IsObject()) {
			for (auto itr = value.MemberBegin(); itr != value.MemberEnd(); ++itr) {
				std::string newPath = path.empty() ? itr->name.GetString() : path + "." + itr->name.GetString();
	...
	static std::optional<std::tuple<std::string, int>> extractKeyAndIndex(const std::string& part) {
	...
	static void UnMadeHttpGetNode(const tfHttpGetNode& nodes, std::string& out) {
		...
		for (const auto& node : nodes) {
			...
			std::vector<std::string> tokens = stl::String::StringSpilt(node.first, ".");
...
public:
	...
	static void Enumerate(const rapidjson::Value& value, int depth = 0) {
		if (value.IsObject()) {
			...
			std::cout << std::string(depth, ' ') << "Bool: " << (value.GetBool() ? "true" : "false") << "\n";
		}
